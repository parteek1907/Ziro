...
export default function TransfersPage() {
  ...
  const FUNDING_SOURCES: FundingSource[] = [
    ...
    { id: "card", name: "JPM Chase", type: "Credit", balance: `$${Math.max(0, cardBalance).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, last4: "3090", holder: "ZAHRA MOHAMADI", cardNumber: "1253 5432 3521 3090", expiry: "09/30", cssClass: "card-blue" },
  ]
  ...
  const maxAvailableUSD = Math.max(vaultBalance, cardBalance)
  const maxAvailable = maxAvailableUSD * (EXCHANGE_RATES[currency] || 1)
  const isInsufficientFunds = grandTotal > maxAvailable

  const selectedSourceBalanceUSD = fundingSource.id === "card" ? cardBalance : vaultBalance
  const selectedSourceBalance = selectedSourceBalanceUSD * (EXCHANGE_RATES[currency] || 1)
  const isSelectedSourceInsufficient = grandTotal > selectedSourceBalance
  ...
  const executePayment = useCallback(async () => {
    ...
    try {
      ...
      deductBalance(usdToDeduct, fundingSource.id)
      ...
      addTransaction({
        ...
        status: "Completed", time: new Date().toISOString(), ref,
      })
  ...
  }, [grandTotal, currency, sym, selectedRecipient, deductBalance, addTransaction, activeUserId, note, totalAmount, fundingSource])
  ...
  return (
    <div className="w-full flex-1 flex flex-col lg:flex-row items-start justify-center px-4 md:px-8 mx-auto relative select-none pt-[10vh] pb-12 gap-6 max-w-6xl">
        ...
        <AnimatePresence mode="popLayout">
          ...
          {step === "amount" && (
            <motion.div key="step-amount" {...motionProps} className={cardClass}>
              ...
              <div className="text-center mb-2">
                ...
                {isInsufficientFunds && amount && <div className="text-xs font-bold text-red-500 mt-2">Insufficient funds (Max: {formatCurrency(maxAvailable)})</div>}
              </div>
              {/* Currency selector */}
          ...
          {step === "source" && (
            <motion.div key="step-source" {...motionProps} className={cardClass}>
              ...
              {isSelectedSourceInsufficient && <div className="text-xs font-bold text-red-500 text-center mb-2">Insufficient funds in {fundingSource.name} (Available: {formatCurrency(selectedSourceBalance)})</div>}
              <button onClick={() => { setStep("route"); fetchRoutes() }} disabled={isSelectedSourceInsufficient} className={ctaClass}>
                <span className="relative z-10">Continue</span>
              </button>
            </motion.div>
          )}
          ...
          {step === "route" && (
            <motion.div key="step-route" {...motionProps} className={cardClass}>
              ...
              <div className="space-y-3 mb-6">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Selected Route</div>
                {allRoutes.length === 0 && (
                ...
                {allRoutes.filter(r => r.route === "BLOCKCHAIN").map((r, i) => {
                  const sel = true // Always selected since it's the only option
                  const isBc = true
                  return (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }}
                      className={`relative p-5 rounded-[24px] transition-all border-2 bg-white shadow-lg border-slate-900 scale-[1.02]`}>
                      <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-slate-900 text-white`}>
                          ...
                          <div>
                            <div className="text-sm font-bold text-slate-800">Blockchain L2</div>
                            <div className="text-xs font-semibold text-slate-500">Instant Settlement</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                          ...
                          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500, damping: 25 }} className="w-6 h-6 rounded-full bg-slate-900 flex items-center justify-center shrink-0"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg></motion.div>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center gap-1.5"><span className="text-[10px] font-bold text-slate-600 uppercase tracking-wide bg-slate-100 px-2.5 py-1 rounded-full">Powered by Ziro</span></div>
                    </motion.div>
                  )
                })}
              </div>
              ...
              {/* Comparison */}
              {(() => {
                const trad = allRoutes.find(r => r.route !== "BLOCKCHAIN")
                if (!trad) return null
                return (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-6 p-4 rounded-[20px] bg-slate-50/80 border border-slate-100 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-500">
                        ...
                        <span className="text-xs font-semibold">Traditional Rails</span>
                      </div>
                      <span className="text-[10px] font-bold text-slate-400 bg-slate-200/50 px-2 py-0.5 rounded-full">Not Recommended</span>
                    </div>
                    <div className="flex items-center justify-between">
                       <div className="text-[11px] font-medium text-slate-400 max-w-[200px] leading-tight">Using legacy banking infrastructure (SWIFT/Local)</div>
                       <div className="text-right">
                         <div className="text-sm font-bold text-slate-400 line-through decoration-slate-300">{trad.estimated_fee === 0 ? "Free" : formatCurrency(trad.estimated_fee)}</div>
                         <div className="text-[10px] font-semibold text-slate-400 mt-0.5">{formatTime(trad.estimated_time_seconds)}</div>
                       </div>
                    </div>
                  </motion.div>
                )
